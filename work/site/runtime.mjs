...
const started=Date.now(),state=C.cleanState(d.state);if(d.mode==='rules'){const result=C.run(d.message,state);result.latencyMs=Date.now()-started;return json(result);}const isDeepSeek=!!env.DEEPSEEK_API_KEY,apiKey=isDeepSeek?env.DEEPSEEK_API_KEY:env.ARK_API_KEY,model=isDeepSeek?(env.DEEPSEEK_MODEL||'deepseek-flash'):env.ARK_MODEL,provider=isDeepSeek?'DeepSeek':'豆包';if(!apiKey||!model)return json({error:'尚未配置模型。请选择规则演示，或在本机连接页完成配置。'},503);
const response=await fetch(isDeepSeek?'https://api.deepseek.com/chat/completions':'https://ark.cn-beijing.volces.com/api/v3/chat/completions',{method:'POST',headers:{'Content-Type':'application/json',Authorization:'Bearer '+apiKey},body:JSON.stringify({model,messages:[{role:'system',content:system},{role:'user',content:JSON.stringify({previousState:state,currentUserMessage:d.message})}],temperature:0,max_tokens:1600,thinking:{type:'disabled'},response_format:{type:'json_object'}}),signal:AbortSignal.timeout(35000)});
if(!response.ok)return json({error:provider+'调用失败（'+response.status+'），请检查服务配置或稍后重试。'},502);const data=await response.json();let output=data.choices?.[0]?.message?.content;if(typeof output!=='string')throw Error('empty');const parsed=JSON.parse(output.replace(/^```(?:json)?\s*|\s*```$/g,''));if(!parsed||typeof parsed!=='object'||Array.isArray(parsed)||!parsed.slots||typeof parsed.slots!=='object'||Array.isArray(parsed.slots)||!['recommend','compare','info','budget','close','greeting','claim','boundary'].includes(parsed.intent))throw Error('schema');const result=C.run(d.message,state,parsed);result.engine=provider+'语义理解 + 确定性规则校验';result.model=model;result.latencyMs=Date.now()-started;result.usage=data.usage||null;return json(result);
