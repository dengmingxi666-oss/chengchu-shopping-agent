...
html = html.replace('href="/"', 'href="./"')
  .replace(/<option value="model">[^<]*<\/option>/, '<option value="model" disabled>模型模式需独立服务端 · 此站未接入</option>');
...
ui = ui.slice(0, statusStart) + "$('mode').value='rules';$('mode').disabled=true;$('mode').dataset.permanentDisabled='true';$('connection').textContent='GitHub Pages · 规则演示';$('model-state').textContent='此入口运行本地规则，不调用模型。模型版需独立服务端；请勿在网页中填写 API 密钥。';" + ui.slice(statusEnd);
